...
OFFICIAL_DICTIONARY = {
    ...
    "AGE": "Applicant's age.",
    "DAYS_EMPLOYED": "Total days of current employment.",
    "total_previous_loans": "Total number of previous loans.",
    ...
    "payment_ratio": "Ratio of Amount Paid vs. Amount Due.",
    "credit_to_income_ratio": "Loan amount divided by income.",
    "annuity_to_income_ratio": "Monthly payment divided by income.",
    "active_loan_ratio": "Proportion of previous loans still active."
}
...
def generate_ai_feedback(probability, shap_evidence, decision):
    ...
    try:
        ...
        return f"⚠️ LLM Offline. Decision: {decision} (Risk: {probability:.2%})."
...
with st.sidebar.expander("💰 Core Financials", expanded=True):
    val_income = st.number_input("Annual Income ($)", value=60000.0)
    val_credit = st.number_input("Requested Loan ($)", value=250000.0)
    val_annuity = st.number_input("Monthly Installment ($)", value=12500.0)
    val_age = st.number_input("Applicant Age", 18, 90, 35)
    val_days_emp = st.number_input("Days Employed (Negative)", value=-1500)
...
with st.sidebar.expander("📝 History Worksheet", expanded=False):
    ...
    val_avg_delay = st.number_input("Avg Delay (Days)", value=2.5)
    ...
    val_bureau_debt = st.slider("Bureau Debt-to-Credit Ratio", 0.0, 1.0, 0.3)
...
if st.button("🚀 EXECUTE RISK ASSESSMENT", type="primary"):
    ...
    # Create 1-row DataFrame (Correcting the Scope)
    ...
    # 5. SHAP ANALYSIS (Moved inside the button logic)
    ...
    # 6. AI Generation (Moved inside the button logic)
